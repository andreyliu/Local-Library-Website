...
class AuthorListViewTest(TestCase):
    ...
    def setUp(self):
        test_user1 = User.objects.create_user(username='testuser1', password='u123456', is_staff=True)
        test_user1.save()
        self.client.login(username='testuser1', password='u123456')
    ...
    def test_pagination(self):
        ...
        self.assertEqual(10, len(response.context['author_list']))
...
class LoanedBookInstancesByUserListViewTest(TestCase):
    ...
    def setUp(self):
        ...
        for book_copy in range(number_of_book_copies):
            ...
            inst.language.set(Language.objects.all())
...
class RenewBookInstancesViewTest(TestCase):
    ...
    def setUp(self):
        ...
        permission = Permission.objects.create(codename='can_mark_returned',
    ...
    def test_form_renewal_date_default(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('renew-book-librarian', kwargs={'pk': self.test_bookinstance1.pk}))
        self.assertEqual(response.status_code, 200)

        expected = datetime.date.today() + datetime.timedelta(weeks=3)
        self.assertEqual(expected, response.context['form'].initial['renewal_date'])
...
class AuthorCreateTest(TestCase):
    ...
    def setUp(self):
        manager = User.objects.create_user(username='manager', password='123456')
        user = User.objects.create_user(username='user', password='123456')
        manager.save()
        user.save()

        perm = Permission.objects.create(codename='can_maintain',
                                         name='Can edit book index',
